...
namespace Driver.Test.Client.Query
{
    public partial class ClientAsync
    {
        ...
        public ClientAsync(ITestOutputHelper output)
        {
            this._output = output;
        ...
        private async Task QueryAsyncTest(string connectString, string db, TDenginePrecision precision)
        {
            ...
            using (var client = await DbDriver.OpenAsync(builder))
            {
                try
                {
                    ...
                    await client.ExecAsync($"create database {db} precision '{PrecisionString(precision)}'");
                    await client.ExecAsync($"use {db}");
                    await client.ExecAsync(this._createTableSql);
                    string insertQuery = string.Format("insert into t1 using all_type tags('{{\"a\":\"b\"}}') {0}",
                        insertSql);
                    await client.ExecAsync(insertQuery);
                    string query = "select * from all_type order by ts asc";
                    using (var rows = await client.QueryAsync(query))
                    {
                        this.AssertColumn(rows);
                        await this.AssertValue(rows, data);
                    }
                }
                catch (Exception e)
                {
        ...
        private async Task QueryWithReqIDAsyncTest(string connectString, string db, TDenginePrecision precision)
        {
            var data = this.GenerateValue(precision, out var insertSql);
            ...
            using (var client = await DbDriver.OpenAsync(builder))
            {
                try
                {
                    await client.ExecAsync($"drop database if exists {db}", ReqId.GetReqId());
                    await client.ExecAsync($"create database {db} precision '{PrecisionString(precision)}'", ReqId.GetReqId());
                    await client.ExecAsync($"use {db}", ReqId.GetReqId());
                    await client.ExecAsync(this._createTableSql, ReqId.GetReqId());
                    string insertQuery = string.Format("insert into t1 using all_type tags('{{\"a\":\"b\"}}') {0}",
                        insertSql);
                    await client.ExecAsync(insertQuery, ReqId.GetReqId());
                    string query = "select * from all_type order by ts asc";
                    using (var rows = await client.QueryAsync(query, ReqId.GetReqId()))
                    {
                        this.AssertColumn(rows);
                        await this.AssertValue(rows, data);
                    }
        ...
        private async Task StmtAsyncTest(string connectString, string db, TDenginePrecision precision)
        {
            ...
            using (var client = await DbDriver.OpenAsync(builder))
            {
                try
                {
                    ...
                    await stmt.PrepareAsync($"insert into ? using all_type tags(?) values({values})");
                    var isInsert = stmt.IsInsert();
                    Assert.True(isInsert);
                    await stmt.SetTableNameAsync("t1");
                    await stmt.SetTagsAsync(new object[] { "{\"a\":\"b\"}" });
                    await stmt.BindRowAsync(data[0]);
                    await stmt.BindRowAsync(data[1]);
                    await stmt.AddBatchAsync();
                    await stmt.ExecAsync();
                    var affected = stmt.Affected();
                    Assert.Equal((long)2, affected);
                    await stmt.PrepareAsync("select * from all_type where ts >= ? order by ts asc");
                    isInsert = stmt.IsInsert();
                    Assert.False(isInsert);
                    await stmt.BindRowAsync(new object[] { data[0][0]! });
                    await stmt.AddBatchAsync();
                    await stmt.ExecAsync();
                    using (var rows = await stmt.ResultAsync())
                    {
                        this.AssertColumn(rows);
                        await this.AssertValue(rows, data);
                    }
        ...
        private async Task StmtWithReqIDAsyncTest(string connectString, string db, TDenginePrecision precision)
        {
            var data = this.GenerateValue(precision, out _);
            ...
            using (var client = await DbDriver.OpenAsync(builder))
            {
                try
                {
                    ...
                    await client.ExecAsync($"use {db}", ReqId.GetReqId());
                    await client.ExecAsync(this._createTableSql, ReqId.GetReqId());
                    var stmt = await client.StmtInitAsync(ReqId.GetReqId());
                    StringBuilder questionMarks = new StringBuilder();
                    var count = data[0].Length;
                    for (int i = 0; i < count; i++)
                    {
        ...
        private async Task StmtBindColumnsAsyncTest(string connectString, string db, TDenginePrecision precision)
        {
            var data = this.GenerateValue(precision, out _);
            ...
            using (var client = await DbDriver.OpenAsync(builder))
            {
                try
                {
                    ...
                    await client.ExecAsync(this._createTableSql);
                    var stmt = await client.StmtInitAsync(ReqId.GetReqId());
                    await stmt.PrepareAsync("insert into ? using all_type tags(?) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
                    var isInsert = stmt.IsInsert();
                    Assert.True(isInsert);
                    await stmt.SetTableNameAsync("t1");
                    await stmt.SetTagsAsync(new object[] { "{\"a\":\"b\"}" });
                    var fields = await stmt.GetColFieldsAsync();
                    await stmt.BindColumnAsync(fields, transposedData);
                    await stmt.AddBatchAsync();
                    await stmt.ExecAsync();
                    var affected = stmt.Affected();
                    Assert.Equal((long)2, affected);
                    ...
                    await stmt.AddBatchAsync();
                    await stmt.ExecAsync();
                    using (var result = await stmt.ResultAsync())
                    {
                        this.AssertColumn(result);
                        await this.AssertValue(result, data);
                    }
                }
                catch (Exception e)
                {
        ...
        private async Task VarbinaryAsyncTest(string connectString, string db)
        {
            ...
            using (var client = await DbDriver.OpenAsync(builder))
            {
                try
                {
                    ...
                    await client.ExecAsync($"use {db}");
                    await client.ExecAsync("create table if not exists test_varbinary(ts timestamp,c1 varbinary(65517))");
                    var stmt = await client.StmtInitAsync(ReqId.GetReqId());
                    await stmt.PrepareAsync("insert into test_varbinary values(?,?)");
                    var isInsert = stmt.IsInsert();
                    Assert.True(isInsert);
                    var fields = await stmt.GetColFieldsAsync();
                    var size = 65517;
                    ...
                    await stmt.BindColumnAsync(fields, new DateTime[] { now }, new byte[][] { data });
                    await stmt.AddBatchAsync();
                    await stmt.ExecAsync();
                    var affected = stmt.Affected();
                    Assert.Equal((long)1, affected);
                    await stmt.PrepareAsync("select * from test_varbinary where c1 = ?");
                    await stmt.BindRowAsync(new object[] { data });
                    await stmt.AddBatchAsync();
                    await stmt.ExecAsync();
                    using (var resut = await stmt.ResultAsync())
                    {
                        var haveNext = await resut.ReadAsync();
        ...
        private async Task InfluxDBAsyncTest(string connectString, string db)
        {
            var builder =
                new ConnectionStringBuilder(connectString);
            using (var client = await DbDriver.OpenAsync(builder))
            {
                try
                {
                    ...
                    await client.ExecAsync($"use {db}");
                    var data =
                        @"http_response,host=host161,method=GET,result=success,server=http://192.168.1.18,status_code=404 response_time=0.003226372,http_response_code=404i,content_length=19i,result_type=""success"",result_code=0i 1648090640000000000
...
jvm_gc_pause_seconds_max,action=end\ of\ minor\ GC,cause=Allocation\ Failure,host=host161,url=http://192.168.17.148:8080/actuator/prometheus gauge=0 1648090640000000000";
                    await client.SchemalessInsertAsync(new string[] { data }, TDengineSchemalessProtocol.TSDB_SML_LINE_PROTOCOL,
        ...
        private async Task TelnetAsyncTest(string connectString, string db)
        {
            var builder =
                new ConnectionStringBuilder(connectString);
            using (var client = await DbDriver.OpenAsync(builder))
            {
                try
                {
                    ...
                    await client.SchemalessInsertAsync(data, TDengineSchemalessProtocol.TSDB_SML_TELNET_PROTOCOL,
        ...
        private async Task SMLJsonAsyncTest(string connectString, string db)
        {
            var builder =
                new ConnectionStringBuilder(connectString);
            using (var client = await DbDriver.OpenAsync(builder))
    ...
    ""tags"": {
       ...
       ""dc"": ""lga""
                    ...
                    await client.SchemalessInsertAsync(data, TDengineSchemalessProtocol.TSDB_SML_JSON_PROTOCOL,
                        TDengineSchemalessPrecision.TSDB_SML_TIMESTAMP_NOT_CONFIGURED, 0, ReqId.GetReqId());
                }
                catch (Exception e)
                {
        ...
        private async Task AssertValue(IRowsAsync rows, object?[][] data)
        {
            for (int i = 0; i < data.Length; i++)
            {
                var haveNext = await rows.ReadAsync();
                Assert.True(haveNext);
